...
func _TestRecursion(t *testing.T) {
	...
	for i := 0; i < noiterations; i++ {
		...
		if err != nil {
			t.Fatalf("Error uploading folder tree %s, error:%s\n", test_f, err)
		}
...
func checkFolder(ftpClient *FTP, f string, homefolder string, t *testing.T) (err error) {
	...
	if err != nil {
		t.Fatalf("Error in Cwd for folder %s: error %s", homefolder, err.Error())
	}
...
func checkintegrityWithPaths(ftpClient *FTP, remotename string, localpath string, istext bool, deleteTemporaryFile bool, t *testing.T) (sizeOriginal int64, sizeOnServer int64, tempFilePath string) {
	...
	if oficp, e = os.Open(tempFilePath); e != nil {
		t.Fatalf("Error opening file %s, error: %s", oficp.Name(), e)
	}
...
func TestConn(t *testing.T) {
	ftpClient := NewFTP(0) // 1 for debugging
	//connect
	_, err := ftpClient.Connect("118.25.187.175", DefaultFtpPort, "")
	if err != nil {
		fmt.Println("The connection failed")
		os.Exit(1)
	}
	defer ftpClient.Quit()
	_, err = ftpClient.Login("wuzl", "dota@11", "")
	if err != nil {
	...
	//rsp, err := ftpClient.Cwd("/home/wuzl/ftp/src")
	//if err != nil {
	//	fmt.Println("The Cwd command failed")
	//	os.Exit(1)
	//}
	//fmt.Println("The current folder is", rsp)

	files, err := ftpClient.Dir("/home/wuzl/ftp/src")
	if err != nil {
		fmt.Println("The Dir command failed")
		os.Exit(1)
	}
